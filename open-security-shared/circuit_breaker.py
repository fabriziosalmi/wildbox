        ...
        async with self._lock:
            ...
            if self.state == CircuitState.OPEN:
                if self._should_attempt_reset():
                    logger.info(
                        f"Circuit breaker '{self.name}': OPEN → HALF_OPEN (attempting recovery)"
                    )
...
OPENAI_BREAKER = CircuitBreaker(
    name="openai", failure_threshold=3, timeout=120, recovery_timeout=60  # 2 minutes
)
...
AWS_API_BREAKER = CircuitBreaker(
    name="aws_api", failure_threshold=10, timeout=180, recovery_timeout=60  # 3 minutes
)

AZURE_API_BREAKER = CircuitBreaker(
    name="azure_api", failure_threshold=10, timeout=180, recovery_timeout=60
)

GCP_API_BREAKER = CircuitBreaker(
    name="gcp_api", failure_threshold=10, timeout=180, recovery_timeout=60
)
