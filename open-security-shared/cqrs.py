...
class CommandBus:
    ...
    async def execute(self, command: TCommand) -> TResult:
        ...
        try:
            ...
            logger.info(f"Command succeeded: {command_type.__name__} ({duration:.3f}s)")
            return result
...
class QueryBus:
    ...
    def register(
        ...
        def decorator(handler: Callable[[TQuery], TResult]) -> Callable:
            ...
            logger.info(
                f"Registered query handler: {query_type.__name__} (TTL: {ttl}s)"
            )
            return handler
    ...
    async def execute(self, query: TQuery) -> TResult:
        ...
        try:
            ...
            logger.info(f"Query succeeded: {query_type.__name__} ({duration:.3f}s)")
            return result
        except Exception as e:
            duration = (datetime.now() - start_time).total_seconds()
            logger.error(
