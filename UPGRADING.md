...
Four changes stop an existing deployment from starting, or change behaviour in a
way that is invisible until it bites. Do them in this order.

### 1. Generate `CSPM_CREDENTIAL_KEY` (required — CSPM will not start without it)
...
```bash
# adds only what is missing, leaves existing secrets alone
make generate-secrets
make validate-secrets      # CSPM_CREDENTIAL_KEY is now a required secret
```
