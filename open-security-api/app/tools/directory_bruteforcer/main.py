...
async def test_path(session: aiohttp.ClientSession, base_url: str, path: str, timeout: int, rate_limiter: RateLimiter) -> DirectoryResult:
    """Test a single path."""
    ...
    async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
        ...
        async def test_with_semaphore(path):
            async with semaphore:
                return await test_path(session, input_data.target_url, path, input_data.timeout, rate_limiter)
